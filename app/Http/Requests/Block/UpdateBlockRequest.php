...
class UpdateBlockRequest extends FormRequest
{
    ...
    public function rules(UrlSecurityService $urlService): array
    {
        ...
        $rules = [
            'type' => ['nullable', 'string', Rule::in(self::SUPPORTED_TYPES)],
        ...
        if ($this->has('config')) {
            if ($type === 'link') {
                $rules['config.title'] = ['required_with:config', 'string', 'max:100'];
                $rules['config.url'] = ['required_with:config', 'string', 'max:500', 'regex:' . self::URL_REGEX];
                ...
                $rules['config.text'] = ['required_with:config', 'string', 'max:120'];
                $rules['config.level'] = ['nullable', 'string', Rule::in(['h1', 'h2', 'h3'])];
            } elseif ($type === 'text') {
                $rules['config.content'] = ['required_with:config', 'string', 'max:1000'];
                $rules['config.align'] = ['nullable', 'string', Rule::in(['left', 'center', 'right'])];
            } elseif ($type === 'divider') {
                $rules['config.style'] = ['nullable', 'string', Rule::in(['line', 'dots', 'space'])];
            } elseif ($type === 'social') {
                $rules['config.platform'] = ['required_with:config', 'string', Rule::in(self::SUPPORTED_PLATFORMS)];
                $rules['config.url'] = ['required_with:config', 'string', 'max:500', 'regex:' . self::URL_REGEX];
            } elseif ($type === 'image') {
                $rules['config.media_id'] = [
                    'required_with:config',
                    'string',
                    'size:26',
                    Rule::exists('profile_media', 'id')->where(fn ($q) => $q->where('profile_id', $userProfile?->id)),
                ];
                ...
                $rules['config.provider'] = ['required_with:config', 'string', Rule::in(['youtube', 'vimeo'])];
                $rules['config.url'] = [
                    'required_with:config',
                    'string',
                    'max:500',
                ...
                $rules['config.provider'] = ['required_with:config', 'string', Rule::in(['spotify', 'apple_music', 'soundcloud'])];
                $rules['config.url'] = [
                    'required_with:config',
                    'string',
                    'max:500',
                ...
                $rules['config.label'] = ['required_with:config', 'string', 'max:100'];
                $rules['config.email'] = ['required_with:config', 'email', 'max:255'];
                ...
                $rules['config.provider'] = ['required_with:config', 'string', Rule::in(['calendly', 'cal'])];
                $rules['config.url'] = [
                    'required_with:config',
                    'string',
                    'max:500',
                ...
                $rules['config.title'] = ['nullable', 'string', 'max:120'];
                $rules['config.items'] = ['required_with:config', 'array', 'min:1', 'max:20'];
                $rules['config.items.*.id'] = ['required_with:config', 'string', 'max:36'];
                $rules['config.items.*.question'] = ['required_with:config', 'string', 'max:200'];
                $rules['config.items.*.answer'] = ['required_with:config', 'string', 'max:1000'];
            } elseif ($type === 'gallery') {
                ...
                $rules['config.title'] = ['required_with:config', 'string', 'max:120'];
                $rules['config.target_date'] = ['required_with:config', 'date'];
                $rules['config.expired_message'] = ['nullable', 'string', 'max:200'];
            } elseif ($type === 'cta') {
                $rules['config.title'] = ['required_with:config', 'string', 'max:120'];
                $rules['config.description'] = ['nullable', 'string', 'max:300'];
                $rules['config.button_label'] = ['required_with:config', 'string', 'max:60'];
                $rules['config.url'] = ['required_with:config', 'string', 'max:500', 'regex:' . self::URL_REGEX];
    ...
    public function messages(): array
    {
        return [
            ...
            'version.required' => 'The block version is required for optimistic concurrency control.',
